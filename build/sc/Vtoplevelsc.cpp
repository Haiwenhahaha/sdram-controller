...
Vtoplevel::Vtoplevel(sc_core::sc_module_name /* unused */)
    ...
    , vlSymsp{new Vtoplevel__Syms(contextp(), name(), this)}
    ...
    , rootp{&(vlSymsp->TOP)}
{
    ...
    sensitive << btn_n_pad_i;
    sensitive << gpio0_io;
    sensitive << gpio1_i;
    sensitive << sdram_dq_pad_io;
...
VL_ATTR_COLD void Vtoplevel::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    if (!sc_core::sc_get_curr_simcontext()->elaboration_done()) {
        vl_fatal(__FILE__, __LINE__, name(), "Vtoplevel::trace() is called before sc_core::sc_start(). Run sc_core::sc_start(sc_core::SC_ZERO_TIME) before trace() to complete elaboration.");
